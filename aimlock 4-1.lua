...
local baseVerticalOffset = 0 -- Initial vertical deviation
local baseHorizontalOffset = 0 -- Initial horizontal deviation
...
local function lookAtLockedTarget()
    if lockedTarget and lockedTarget.Character and lockedTarget.Character:FindFirstChild("Head") then
        ...
        local targetPos = predictTargetPosition(lockedTarget) or lockedTarget.Character.Head.Position
        local distance = (playerPos - targetPos).magnitude

        -- Proportional deviation
        local verticalOffset = baseVerticalOffset * (distance / 50)
        local horizontalOffset = baseHorizontalOffset * (distance / 50)

        targetPos = targetPos + Vector3.new(horizontalOffset, verticalOffset, 0)
